...
Alpine.data('app', () => ({
  ...
  _renderCookArtifactHtml(model) {
    // Thin caller (Phase 18 / Plan 18-01): the cook-sheet CSS, runtime, and
    // document skeleton now live in cook-render.js (single source of truth). Here
    // we only build the escaped JSON data island and the title, then delegate.
    //
    // Data-island escaping stays in the CALLER (every < -> <) so a recipe
    // step containing </script> in ANY casing can't close the tag early;
    // JSON.parse re-expands < transparently. The viewer (18-04) supplies its own
    // data source the same way. Recipe text is rendered client-side via
    // textContent (NOT innerHTML), so malformed-HTML/XSS is structurally avoided
    // (RESEARCH escaping recommendation; threat T-06-03).
    const json = JSON.stringify(model).replace(/</g, '\\u003c');
    const dataIsland = `<script type="application/json" id="cook-data">${json}</script>`;
    const titleDay = String(model.dayLabel || 'Cook');
    // Share-link base (Plan 18-04): the generated sheet runs inside a `blob:`
    // document where `new URL('cook.html', location.href)` THROWS, so the blob
    // cannot resolve the viewer URL relatively. Resolve it HERE in the parent
    // (which has a real https/http origin) and embed it for the in-sheet Share
    // control. Falls back to '' (relative) if resolution somehow throws.
    let shareBase = '';
    try { shareBase = new URL('cook.html', window.location.href).href; } catch (e) { shareBase = ''; }
    return renderCookDocument({ dataIsland, titleDay, shareBase });
  },
